...
- (IBAction)HourEndEditing:(id)sender {
    ...
    if (![self.hour.text isEqualToString:@"0"]&&![self.hour.text isEqualToString: @"00"]&&hourInt == 0) {
        hourInt = 1000;
    }
...
- (IBAction)MinuteEndEditing:(id)sender {
    ...
    if ((![self.minute.text isEqualToString:@"0"]||![self.minute.text isEqualToString: @"00"])&&minuteInt == 0) {
        minuteInt = 1000;
    }
...
- (void)AddTaskTakerViewController:(AddTaskTakerTableViewController *)controller didFinishAddingTaskTaker:(Contact *)contact{
    self.taskTaker.text = contact.name;
    [self dismissViewControllerAnimated:YES completion:nil];
}
